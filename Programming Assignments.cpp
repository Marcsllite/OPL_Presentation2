...
int main() {
	...
	// Regular example (from 9-2 in notes)
	CEK test2(new Application(
		new Abstraction("x", 
			new Application (
				new Abstraction("y", 
					new PrimitiveOp("add", new ConstantList(new Variable("x"), new ConstantList(new Variable("y"), new NullList())))
				), new Number(8))
		), new Number(7))
	);  // ((LambdaX. {(LambdaY. {X + Y})8})7) where {} = body of an abstraction ; = 15
	...
	CEK test7(new Application(new Application(new Abstraction("x", new Variable("x")), new Number(2)), new Number(21)));  // (((LambdaX. X)2) 21) (ERROR: appliedTo cannot be a number)
	...
	cout << "Test1 21: " << test1.eval()->toString() << endl;
	cout << "Test2 ((Lamx. ((Lamy. (add x y)) 8)) 7): " << test2.eval()->toString() << endl;
	cout << "Test3 x: " << test3.eval()->toString() << endl;
	cout << "Test4 (1 2): " << test4.eval()->toString() << endl;
	cout << "Test5 (x 2): " << test5.eval()->toString() << endl;
	cout << "Test6 ((Lamx.x) 2): " << test6.eval()->toString() << endl;
	cout << "Test7 (((Lamx.x) 2) 21): " << test7.eval()->toString() << endl;
	cout << "Test8 ((add1 20) 2): " << test8.eval()->toString() << endl;
	//cout << "Test9: " << test9.eval()->toString() << endl;
	cout << "Test10 (Lamx. 21): " << test10.eval()->toString() << endl;
